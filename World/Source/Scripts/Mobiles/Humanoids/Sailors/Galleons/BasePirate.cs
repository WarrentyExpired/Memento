...
namespace Server.Mobiles 
{
	public class BasePirate : BaseCreature 
	{
		...
		public void DressUpCrewMember( BaseCreature bc, Mobile captain )
		{
			...
			else if ( captain is PirateCult )
			{
				...
				switch ( Utility.RandomMinMax( 0, 3 ) )
				{
					...
					case 3: bc.AddItem( new HoodedMantle() ); break;
				}
				MorphingTime.ColorMyClothes( bc, captain.SpeechHue, 0 );
				bc.AddItem( new ElvenBoots( 0x83A ) );
			...
			else if ( captain is SailorGuards || captain is SailorElfGuards || captain is SailorOrkGuards )
			{
				...
				if ( captain is SailorElfGuards ){ bc.HairHue = Utility.RandomHairHue(); bc.Hue = Utility.RandomSkinColor(); }
			}
			else if ( captain is SailorMerchant || captain is SailorElf )
			{
				DressSailor( bc );
				if ( captain is SailorElf ){ bc.HairHue = Utility.RandomHairHue(); bc.Hue = Utility.RandomSkinColor(); }
		...
		public void TitleGuards( Mobile captain )
		{
			...
			else if ( captain.Land == Land.Kuldar ){ title = 20; }
		...
		public void DressGuards( BaseCreature bc, Mobile captain )
		{
			...
			MorphingTime.ColorMyClothes( bc, clothColor, 0 );
		...
		public void DressSailor( BaseCreature bc )
		{
			...
			MorphingTime.ColorOnlyClothes( bc, Utility.RandomColor(0), 0 );
		}
		...
		public void SizeUpCrewMember( BaseCreature bc, bool evil, int link, Mobile captain, string toss )
		{
			...
			bc.SetSkill( SkillName.Marksmanship, 66.0, 97.5 );
			bc.SetSkill( SkillName.MagicResist, 25.0, 47.5 );
			bc.SetSkill( SkillName.Tactics, 65.0, 87.5 );
			bc.SetSkill( SkillName.FistFighting, 66.0, 97.5 );
		...
		{
			BaseBoat boat = new GalleonWreckedBarbarian();
			switch( Utility.RandomMinMax( 1, 7 ) )
			{
				case 1: boat = new GalleonWreckedRoyal();		break;
				case 2: boat = new GalleonWreckedExotic();		break;
				case 3: boat = new GalleonWreckedLarge();		break;
				case 4: boat = new GalleonRuinedBarbarian();	break;
				case 5: boat = new GalleonRuinedRoyal();		break;
				case 6: boat = new GalleonRuinedExotic();		break;
			}
