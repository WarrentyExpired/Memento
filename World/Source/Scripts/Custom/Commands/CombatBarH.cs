...
{
    public class CombatBarH
    {
        public static void Initialize()
        {
            CommandSystem.Register("combatbarh", AccessLevel.Player, new CommandEventHandler(ToolBar_OnCommand));
        }
        ...
        public static void Refresh(Mobile from, bool force = false)
        {
            ...
            {
                if (pm.HasGump(typeof(CombatBarGumpH)) || force)
                {
                    pm.CloseGump(typeof(CombatBarGumpH));
                    pm.SendGump(new CombatBarGumpH(pm));
                }
            }
        }
        public class CombatBarGumpH : Gump
        {
            private PlayerMobile m_Player;
            public CombatBarGumpH(PlayerMobile player) : base(100, 100)
            {
                m_Player = player;
                Closable = true;
                Disposable = true;
