...
namespace Server.Regions
{
	public class HouseRegion : BaseRegion
	{
		...
		public BaseHouse Home{ get{ return m_House; } }
		...
		{
			if ( ( m.Map == Map.Underworld) && ( m.X > 1630 ) && ( m.Y < 1080 ) )
			{
				global = LightCycle.JailLevel;
			}
			else if ( ( m.Map == Map.Underworld) )
			{
				global = LightCycle.DungeonLevel;
			}
			else if ( m.Map == Map.Lodor && m.X > 6446 && m.Y > 3056 )
			{
				global = LightCycle.DungeonLevel;
			}
