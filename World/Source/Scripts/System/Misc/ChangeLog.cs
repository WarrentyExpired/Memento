...
namespace Server.Misc
{
	class ChangeLog
	{
		...
		{
			if (page < 0 || page >= RELEASE_COUNT) return string.Empty;

			var releaseIndex = RELEASE_COUNT - 1 - page;
			var builder = new StringBuilder();
			AppendRelease(builder, releaseIndex);

			return builder.ToString();
		}

		private static void AppendRelease(StringBuilder builder, int releaseIndex)
		{
			switch (releaseIndex)
			{
				...
				case 9:
					...
					builder.Append("- Commands - Add [Rename command to rename a container<br>");
					...
					builder.Append("- Misc - Permadeath characters can now be immediately deleted<br>");
					...
					builder.Append("- Skill - Provocation success cooldown is now 6 seconds, down from 10<br>");
					...
					builder.Append("- Misc - Town Crier should now appropriately put a space after player title and events<br>");
				...
				case 11:
					...
					builder.Append("- Misc - Add rejection message when turning in an incomplete golem manual<br>");
					builder.Append("- Library - Remove outdated 'Basics' book<br>");
					...
					builder.Append("- Item - Hue vacuum tube is still usable on, but no longer removes the dye from stackable items<br>");
					...
					builder.Append("- Item - Only small pumpkins are stackable<br>");
					builder.Append("- Item - Only one pair of pants can be equipped at a time<br>");
					builder.Append("- Item - Fix an issue where weapon damage would never deduct max durability<br>");
					builder.Append("- Misc - Fix a newly introduced issue (/crash) where spells cost +1 circle higher<br>");
					...
					builder.Append("- Skill - Fix issue causing Parrying activation to penalize 125 high skill users who have under 80 dexterity<br>");
