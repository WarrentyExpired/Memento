      ...
      protected override void OnTarget( Mobile from, object targeted )
      {
        ...
        else if ( targeted is Mobile )
        {
          ...
          if ( targeted == from )
          {
            ...
            {
              from.SendLocalizedMessage( 500613 );
              m_Instrument.PlayInstrumentBadly( from );
              m_Instrument.ConsumeUse( from );
            }
            else
          ...
          else
          {
            ...
            {
              from.SendLocalizedMessage( 1049526 );
              m_SetSkillTime = true;
            }
            else if ( targ is BaseCreature && ((BaseCreature)targ).BardPacified )
            {
              from.SendLocalizedMessage( 1049527 );
              m_SetSkillTime = true;
            }
            ...
            else
            {
              ...
              {
                from.SendLocalizedMessage( 1049531 );
                m_Instrument.PlayInstrumentBadly( from );
                m_Instrument.ConsumeUse( from );
              }
              else
              {
                ...
                if ( targ is BaseCreature )
                {
                  ...
                  from.SendLocalizedMessage( 1049532 );
                  targ.Combatant = null;
                  targ.Warmode = false;
                  bc.Pacify( from, DateTime.Now + TimeSpan.FromSeconds( seconds ) );

                  // BULLSEYE LOGIC
                  double skill = from.Skills[SkillName.Peacemaking].Value;
                  double bullseyeChance = (skill - (diff - 25.0)) / 50.0;
                  ...
                  {
                    from.SendMessage("You failed to entrance the target; its focus remains sharp despite being calmed.");
                  }
                }  
