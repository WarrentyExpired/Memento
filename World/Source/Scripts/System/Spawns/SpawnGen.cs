	...
	public class SpawnGenerator
	{
		...
		public static void Parse( Mobile from, string filename )
		{
			string monster_path1 = Path.Combine( Core.BaseDirectory, "Data/Spawns" );
				if ( filename == "Spawns.map" ){ monster_path1 = Path.Combine( Core.BaseDirectory, "Info/Spawns" ); }
				else if ( filename == "Merchant.map" ){ monster_path1 = Path.Combine( Core.BaseDirectory, "Info/Spawns" ); }
		...
		public static double CustomTime( double val )
		{
			int min = MySettings.S_SpawnMin;
			int max = MySettings.S_SpawnMax;
