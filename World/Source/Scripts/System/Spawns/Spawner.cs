...
//				list.Add( 1060660, "team\t{0}", m_Team );
		...
		public static void SpreadOut( Mobile m )
		{
			...
			else
			{
				...
				if ( m.WhisperHue == 999 && ( m is Balron || m is Daemon || m is RidingDragon || m is YoungDragon || m is Dragons || m is Wyrms ) ){ m.CanSwim = true; }
			}

			if ( Worlds.IsMassSpawnZone( m.Map, m.X, m.Y ) ){ m.Delete(); }
		}
		...
		public void Spawn( int index )
		{
			...
			if ( ent is Mobile || ent is BaseSpawner )
			{
				...
				if ( ent is BaseSpawner )
					m = SpawnList.RandomSpawn( (Item)ent, this.Land, loc, this.Map );
				else
					m = (Mobile)ent;
				...
				if ( !MySettings.S_Scary && SpawnID == 8888 )
				{
					...
					else if ( m is Hydra ){ m.Delete(); m = new Wyverns(); }
					else if ( m is Daemon ){ m.Delete(); m = new Gargoyle(); }
					else if ( m is Balron ){ m.Delete(); m = new Demon(); }
					else if ( m is AncientEnt ){ m.Delete(); m = new Ent(); }
					else if ( m is AsianDragon ){ if ( Utility.RandomBool() ){ m = new Pixie(); } else { m = new Faerie(); } }
					else if ( m is Angel ){ if ( Utility.RandomBool() ){ m = new Pixie(); } else { m = new Faerie(); } }
					else if ( m is Archangel ){ m.Delete(); if ( Utility.RandomBool() ){ m = new Pixie(); } else { m = new Faerie(); } }
				}
				...
				Server.Mobiles.PremiumSpawner.SpreadOut( m );
        ...
        public Point3D GetSpawnSpotLandDungeon()
        {
			...
			return Server.Misc.Worlds.GetRandomLocation( Land, "land" );
		}
