...
namespace Server.Misc
{
	...
	{
		private static Point2D[] m_Lodor = new Point2D[]
			{
		...
		private static Point2D[] m_IslesDread = new Point2D[]
			{
				new Point2D( 649, 909 ), new Point2D( 148, 1243 ), new Point2D( 453, 1284 ),
				new Point2D( 277, 948 )
			};
		private static Point2D[] m_SavagedEmpire = new Point2D[]
			{
		...
		public static void EventSink_Login( LoginEventArgs e )
		{
			...
			if ( from.Land == Land.Lodoria )
				list = m_Lodor;
			else if ( from.Land == Land.Serpent )
				list = m_SerpentIsland;
			else if ( from.Land == Land.IslesDread )
				list = m_IslesDread;
			else if ( from.Land == Land.Savaged )
				list = m_SavagedEmpire;
			else if ( from.Land == Land.Kuldar )
				list = m_Bottle;
			else if ( from.Land == Land.UmberVeil )
				list = m_Umber;
			else if ( from.Land == Land.Ambrosia )
				list = m_Ambrosia;
		...
		public static void CabinGoneByeBye( Mobile from, string world )
		{
			...
			if ( world == "the Land of Lodoria" ){ list = m_Lodor; map = Map.Lodor; }
