...
namespace Server.Items
{
	public class BoatDoor : Item
	{
		...
		{
			string sWorld = Server.Lands.LandName( Server.Lands.GetLand( m.Map, m.Location, m.X, m.Y ) );
	...
	public class DeckDoor : Item
	{
		...
		public void DoDeckDoor( Mobile m )
		{
			...
			if ( m is PlayerMobile )
			{
				sCabinDoor = ((PlayerMobile)m).CharacterBoatDoor;
					...
					foreach ( Item item in World.Items.Values )
					{
						if ( item is BoatDoor )
						{
							...
							if ( 	item != null && 
									...
									Server.Lands.LandName( Server.Lands.GetLand( item.Map, item.Location, item.X, item.Y ) ) == sWorld 
							)
							{
								loc = item.Location;
								map = item.Map;
				...
				((PlayerMobile)m).CharacterBoatDoor = null;
			}
		}
		...
		{
			if ( Server.Misc.Worlds.GetRegionName( from.Map, from.Location ) == "the Ship's Lower Deck" && from is PlayerMobile )
			{
				DoDeckDoor( from );
				return false;
			}
