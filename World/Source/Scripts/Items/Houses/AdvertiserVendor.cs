			...
			private void AddDetails( int index )
			{
				try{
					if ( index < m_List.Count )
					{
						...
						else if ( ( pv.Map == Map.Vaelen ) && ( pv.X > 5218 ) && ( pv.Y > 1036 ) && ( pv.X < 5414 ) && ( pv.Y < 1304 ) ) { vMap = "Umbra Cave"; xSet = 3370; ySet = 1553; mSet = Map.Vaelen; }
						else if ( ( pv.Map == Map.Vaelen ) && ( pv.X > 6548 ) && ( pv.Y > 3812 ) && ( pv.X < 6741 ) && ( pv.Y < 4071 ) ) { vMap = "Shipwreck Grotto"; xSet = 318; ySet = 1397; mSet = Map.Vaelen; }
						else if ( ( pv.Map == Map.Vaelen ) && ( pv.X > 860 ) && ( pv.Y > 3184 ) && ( pv.X < 2136 ) && ( pv.Y < 4090 ) ) { vMap = "Umber Veil"; xSet = pv.X; ySet = pv.Y; mSet = Map.Vaelen; }
						else if ( ( pv.Map == Map.Vaelen ) && ( pv.X > 5129 ) && ( pv.Y > 3062 ) ) { vMap = "Ambrosia"; xSet = pv.X; ySet = pv.Y; mSet = Map.Vaelen; }
						else if ( ( pv.Map == Map.Vaelen ) && ( pv.X > 5793 ) && ( pv.Y > 2738 ) && ( pv.X < 6095 ) && ( pv.Y < 3011 ) ) { vMap = "Moon of Luna"; xSet = 3696; ySet = 519; mSet = Map.Vaelen; }
						else if ( ( pv.Map == Map.SerpentIsland ) && ( pv.X > 1875 ) )
						{
							if ( ( pv.X > 1949 ) && ( pv.Y > 1393 ) && ( pv.X < 2061 ) && ( pv.Y < 1486 ) ){ xSet = 1863; ySet = 1129; vMap = "Vaelen"; mSet = Map.Vaelen; }
							...
							else if ( ( pv.X > 1930 ) && ( pv.Y > 1890 ) && ( pv.X < 2022 ) && ( pv.Y < 1997 ) ){ xSet = 3884; ySet = 2879; vMap = "Vaelen"; mSet = Map.Vaelen; }
						}
						...
						else if ( ( pv.Map == Map.Underworld ) && ( pv.X > 1630 ) )
						{
							...
							else if ( ( pv.X > 1877 ) && ( pv.Y > 187 ) && ( pv.X < 2033 ) && ( pv.Y < 302 ) ){ xSet = 1944; ySet = 3377; vMap = "Umber Veil"; mSet = Map.Vaelen; }
							else if ( ( pv.X > 2108 ) && ( pv.Y > 190 ) && ( pv.X < 2269 ) && ( pv.Y < 305 ) ){ xSet = 1544; ySet = 1785; vMap = "Serpent Island"; mSet = Map.SerpentIsland; }
							else if ( ( pv.X > 1656 ) && ( pv.Y > 335 ) && ( pv.X < 1807 ) && ( pv.Y < 443 ) ){ xSet = 2059; ySet = 2406; vMap = "Vaelen"; mSet = Map.Vaelen; }
							...
							else if ( ( pv.X > 1657 ) && ( pv.Y > 496 ) && ( pv.X < 1807 ) && ( pv.Y < 606 ) ){ xSet = 2181; ySet = 1327; vMap = "Vaelen"; mSet = Map.Vaelen; }
							...
							else if ( ( pv.X > 1659 ) && ( pv.Y > 953 ) && ( pv.X < 1809 ) && ( pv.Y < 1059 ) ){ xSet = 5390; ySet = 3280; vMap = "Ambrosia"; mSet = Map.Vaelen; }
						...
						else if ( pv.Map == Map.Vaelen && pv.X > 5125 && pv.Y > 3038 && pv.X < 6124 && pv.Y < 4093 ){ vMap = "Ambrosia"; xSet = pv.X; ySet = pv.Y; mSet = Map.Vaelen; }
						else if ( pv.Map == Map.Vaelen && pv.X > 699 && pv.Y > 3129 && pv.X < 2272 && pv.Y < 4095 ){ vMap = "Umber Veil"; xSet = pv.X; ySet = pv.Y; mSet = Map.Vaelen; }
						else if ( pv.Map == Map.Vaelen && pv.X > 6127 && pv.Y > 828 && pv.X < 7168 && pv.Y < 2738 ){ vMap = "Bottle World"; xSet = pv.X; ySet = pv.Y; mSet = Map.Vaelen; }
						else { vMap = "Vaelen"; xSet = pv.X; ySet = pv.Y; mSet = Map.Vaelen; }
