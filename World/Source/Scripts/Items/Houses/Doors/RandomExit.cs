		...
		public override bool OnMoveOver( Mobile m )
		{
			if ( m is PlayerMobile )
			{
				...
				if ( sPublicDoor != null )
				{
					...
					Point3D p = Worlds.GetRandomLocation( Server.Lands.GetLand( mWorld, origin, mX, mY ), "land" );
					Map map = Worlds.GetMyDefaultMap( Server.Lands.GetLand( mWorld, origin, mX, mY ) );
