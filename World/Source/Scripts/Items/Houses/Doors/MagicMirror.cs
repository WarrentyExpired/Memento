	...
	public class MagicMirror : Item
	{
		...
		public static void SetMirrors()
		{
			...
			for ( int i = 0; i < mirrors.Count; ++i )
			{
				...
				Point3D p = Worlds.GetRandomLocation( item.Land, "land" );
				Map map = Worlds.GetMyDefaultMap( item.Land );
					mirror.MirrorX = p.X;
					mirror.MirrorY = p.Y;
					mirror.MirrorZ = p.Z;
				...
				{
					if ( item.Land == Land.IslesDread && MirrorIslesDread < 2 )
					{
						MirrorIslesDread++;
						item.Visible = true;
					}
					else if ( item.Land == Land.Savaged && MirrorSavagedEmpire < 2 )
					{
						MirrorSavagedEmpire++;
						item.Visible = true;
					}
					else if ( item.Land == Land.Lodoria && MirrorLodor < 2 )
					{
						MirrorLodor++;
						item.Visible = true;
					}
					else if ( item.Land == Land.Serpent && MirrorSerpentIsland < 2 )
					{
						MirrorSerpentIsland++;
						item.Visible = true;
					}
		...
		public virtual void DoTeleport( Mobile m )
		{
			...
			{
				foreach ( Item item in World.Items.Values )
				if ( item is MagicMirror && item != this && item.Visible && item.Land == Land )
				{
